...
public class ConnectToUser3 implements Runnable {
    ...
    static DatagramSocket3 sending_socket, receiving_socket;

    static class EstConnection implements Runnable {
        static DatagramSocket3 two_receiving_socket;
        ...
        public EstConnection() {
            try { //Try set up receiving port
                this.two_receiving_socket = new DatagramSocket3(ConnectToUser3.ReceivePort-2); //Receive heartbeat on port 10009
            } catch (SocketException e) {
                ConnectToUser3.connected = false;
                ConnectToUser3.connection_failure_reason = "Unable to open heartbeat port " + (ConnectToUser3.ReceivePort-2);
            }
        }
        ...
        static class SendHeartBeat implements Runnable {
            ...
            @Override
            public void run() {
                if (debug) {System.out.println("Beginning sending of packets to " + RecipientIP.getHostName() + ":" + (ConnectToUser3.ReceivePort-2));}
                while (s_a_r_running) {
                    byte[] buffer = new byte[]{(byte) code_to_send};
                    DatagramPacket packet = new DatagramPacket(buffer, 1, RecipientIP, ConnectToUser3.ReceivePort-2); //Both users send from any available port to 10011
                    try {
                        if (debug) {System.out.println("Port [" + sending_socket.getLocalPort() + "] Sending data to " + RecipientIP.getHostName() + ":" + (ConnectToUser3.ReceivePort-2));}
    ...
    public ConnectToUser3(String IP) {
        connected = true;
        try {
            RecipientIP = InetAddress.getByName(IP); //Grab IP from input
            receiving_socket = new DatagramSocket3(ReceivePort);
